...
def extend_maps(word2id, tag2id, for_crf=True):
    if '<unk>' not in  word2id.keys():
        word2id['<unk>'] = len(word2id)
    if '<pad>' not in word2id.keys():
        word2id['<pad>'] = len(word2id)
    if '<unk>' not in tag2id.keys():
        tag2id['<unk>'] = len(tag2id)
    if '<pad>' not in tag2id.keys():
        tag2id['<pad>'] = len(tag2id)
    # 如果是加了CRF的bilstm  那么还要加入<start> 和 <end>token
    if for_crf:
        if '<start>' not in  word2id.keys():
            word2id['<start>'] = len(word2id)
        if '<end>' not in  word2id.keys():
            word2id['<end>'] = len(word2id)
        if '<start>' not in tag2id.keys():
            tag2id['<start>'] = len(tag2id)
        if '<end>' not in tag2id.keys():
